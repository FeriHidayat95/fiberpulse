...
import { X, MapPin, Save, Wifi, Server, Loader2, Crosshair } from 'lucide-react';
import { Modal } from './ui/Modal';
import toast from 'react-hot-toast';

export const EditOdpModal = ({ isOpen, onClose, onSuccess, odpData }) => {
  ...
  useEffect(() => {
    if (isOpen && odpData) {
      setFormData({
        name: odpData.name || '',
        dusun: odpData.dusun || '',
        total_ports: odpData.total_ports || 8,
        latitude: odpData.latitude || '',
        longitude: odpData.longitude || ''
      });
    ...
    if (checked) {
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          toast.success(`Lokasi GPS diperbarui (Akurasi: ±${Math.round(pos.coords.accuracy)}m)`);
        },
        (err) => {
          setIsGettingLocation(false);
          setFormData(prev => ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/odps/${odpData.id}`, {
        method: 'PUT',
      ...
      if (!res.ok || !json.success) {
        throw new Error(json.message || 'Gagal memperbarui ODP');
      }

      toast.success(`ODP ${formData.name} berhasil diperbarui!`);
      if (onSuccess) onSuccess();
      onClose();
    } catch (err) {
      toast.error(err.message || 'Gagal memperbarui ODP');
    } finally {
  ...
  const customTitle = (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div>
        <h2 style={{ fontSize: '1rem', fontWeight: 800, margin: 0, color: '#152C4A' }}>Edit ODP — {odpData.name}</h2>
        <p style={{ fontSize: '0.75rem', color: '#64748B', margin: '2px 0 0 0', fontWeight: 500 }}>Perbarui informasi titik dan kapasitas port ODP</p>
      </div>
    </div>
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={customTitle} footer={customFooter} maxWidth="480px">
      <form id="editOdpForm" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
        <div>
        ...
        <div style={{
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700, color: '#152C4A', margin: 0 }}>
            ...
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
              <Crosshair size={14} color="#2563EB" /> Gunakan Lokasi Saya Saat Ini (GPS Otomatis)
            </span>
          </label>
          {isGettingLocation && (
